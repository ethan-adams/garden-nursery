...
## 2026-07-11 - Visual identity: cozy pixel art (CC0/CC-BY), single coherent source

**Decision:** The game's near-term visual identity is **cozy pixel art**, classic cozy-sim / Stardew-adjacent, sourced from one coherent openly-licensed set (CC0 preferred, CC-BY acceptable with attribution), curated hard for coherence over fidelity (issue #100). Chosen by Ethan from an art-direction fork (Kenney-flat vs pixel vs bespoke-SVG vs defer). This **supersedes the art-bible's "painted botanical illustration, not generic farm sim pixel art" pillar for the current gate**: VISION.md already frames the art bible as "the eventual bar, not the current gate," so the painterly-botanical target is deferred to a possible later commission (Ethan has friends who make game art), and pixel art is what ships now. The rejected placeholder look — the single baked bespoke `hush-arbor-yard.png` SVG plus text station labels — is what this replaces.

**Reason:** The build's graphics were the standing complaint since the 2026-07-10 vision reset, and the felt-experience gate for `0.3 Make It Real` ("feels like a game, not forms") stalls on the debug-scene look. Free packs first was already decided; the open question was *which style*. Pixel art gives a coherent, warm, tactile, plant-forward identity from a single source with the lowest coherence risk when curation stays to one pack, and it is the most recognizable cozy-sim register — the one most likely to keep player-zero (Ethan) engaged.

**Consequence (first pass shipped 2026-07-11):** The chosen source is **Kenney's "Tiny Farm" pack** (130 assets, 16px tiles, **CC0** — no attribution required), from https://kenney.nl/assets/tiny-farm, committed under `godot/assets/art/tiny-farm/` with its `License.txt`. The yard background (`hush-arbor-yard.png`) is now composed from those tiles by `godot/tools/compose_yard.gd` (native 320x200, displayed 4x = 1280x800), which replaces the retired SVG rasterizer (`rasterize_art.gd` and the `hush-arbor-yard.svg`/`gardener-player.svg` sources are deleted). Rendering is pixel-perfect via nearest-neighbour `texture_filter` + 4x scale on the yard and player sprites (scoped to those sprites, not the UI). The player is Tiny Farm's farmer tile (`player-pixel.png`). The floating `SignLabel` station plaques are removed — stations read from their pixel prop silhouettes, and the onboarding flow names each one contextually. `agent-check` swapped its SVG/sign-label guards for pixel-pipeline guards (CC0 license present, `compose_yard.gd` composes from the pack, both sprites use nearest filtering). This is a **first pass**: the plant-stand centrepiece reads well; the ledger (crate) and journal (barrel) props are the weak mappings — Tiny Farm is farm-, not office-, themed — and want Ethan's art-director eye to refine. The art bible is annotated to point here.

## 2026-07-16 - The harness loops on VISION.md; issues are evidence, not the driver

**Decision:** Ethan's generalized harness repo (`~/dev/harness`, paired with the `~/dev/wiki` knowledge corpus) now drives this project's work, and the paradigm changes: the harness loops autonomously toward `docs/VISION.md` — each iteration picks and ships the slice that most increases "feels like a real game" (player-zero rule) — rather than treating the GitHub issue queue as the source of truth. Ethan's words: assume some or all of the open issues are bunk; the new paradigm is looping to make the game the vision describes. Issues stay useful as durable work records: an iteration that lands on a real open issue claims and closes it through the harness queue (`bin/queue`), and stale or falsified issues get closed or corrected as they're encountered.

**Reason:** The codex-era habit this retires (measuring progress by issues closed) was already rejected in the 2026-07-10 vision reset; this extends the same logic to work selection. The 2026-07-16 ground-truth survey confirmed the queue is partly stale — #91 was listed as "current next" in `docs/issue-backlog.md` while already fixed in code — while the real felt gaps (frozen calendar, unseeded RNG, god-object state) are exactly what VISION.md's pillars point at. Vision-first selection with code-verified evidence beats trusting a drifted backlog.

**Consequence:** Each loop iteration: survey ground truth where docs are suspect → pick the highest felt-impact slice → implement under the fast-ship contract (local gates, one `Harness-Managed: true` commit, push) → sync issues/docs to reality → log evidence to the wiki. Stop-and-ask boundaries in `CLAUDE.md` are unchanged. First iteration under this paradigm: the full-year calendar (#99).

## 2026-07-16 - Hush Arbor's year: 24 wrapping weeks, seasonal signals, events that re-arm

**Decision:** The season calendar (#99) is a **24-week year** (four seasons, six weeks each, phase labels like "early spring" / "mid winter") that **wraps**: week 25 is week 1 of Year 2, and the forecast summary announces "Year 2" so the repeat is a rhythm, not a bug. Market signals carry an optional `seasons` tag (family names: spring/summer/autumn/winter, derived from the last word of the season label) and only circulate in their seasons — the board now reads differently in winter than in spring; untagged signals circulate year-round. Community events live on week-of-year windows and **re-arm each year** (resolution keys are `id#y<N>` from Year 2 on; Year 1 keeps bare ids so existing saves stay valid). Hush Arbor now has four events across the year (seed swap, porch supper week, orchard gleaning, longest-night lanterns) and seven week outcomes. The longest-night beat is the first place the restrained-magic pillar surfaces in play (glowing/magical traits finally matter).

**Reason:** The five-entry calendar froze on "late autumn" from week 5 forever — the single biggest cause of the world feeling dead, found in the 2026-07-10 critical review. A wrapping year is the smallest design that makes time never freeze; per-year event re-arm keeps every year playable rather than front-loading all novelty into year one.

**Consequence:** `year_length`/`week_of_year`/`year_number`/`season_family`/`available_market_signals` are the new run-state vocabulary; data validation now asserts a contiguous ≥20-week calendar through all four seasons, ≥3 events inside the year, and valid season tags. The JS rules mirror got the same wrap fix (noting the mirror remains a liability per the 2026-07-10 review). One data drift fixed en route: the old week-5 entry pointed at "old-orchards" (a soil name) instead of the plant trait "old-orchard", so it could never match a plant. Writing note: 19 new forecast lines, 3 events, and 4 outcomes were written to the creative bar — Ethan edits for taste when he wants.
