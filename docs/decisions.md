...
**Consequence:** SVGs are the editable art source; `godot/tools/rasterize_art.gd` regenerates the committed PNGs (import-pipeline rule unchanged). UI text is Alegreya/Alegreya Sans (SIL OFL 1.1, license texts committed beside the fonts). The week-outcome header overflow (filed from the first screenshot run) is fixed by moving visits into a stats box and trimming the region line. `agent-check` now guards the theme wiring, font licenses, and sign-label layer in place of the old hex-chip assertions. The stand overlay stays a full-screen surface for now; making it feel more like standing at a market table is future product work, not theme work.
